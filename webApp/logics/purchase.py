...
class Processor(ProcessorBase):
	...
	def _fn_html_top(self, chain_env):
		...
		#Fetch engineers.
		status_engineer, render_param['purchase.enumPurchases'] = self._fn_enum_purchases(chain_env)
	...
	def _fn_enum_purchases(self, chain_env):
		...
		if dbcur:
			...
			if ("engineer_client_name" in args):
				whereClause.append("((`FQUC`.`id` IS NULL AND `FQCL`.`name` REGEXP \'^.*" + args['engineer_client_name'] + ".*$\') OR (`FQUC`.`id` IS NOT NULL AND `FQUC`.`name` REGEXP \'^.*" + args['engineer_client_name'] + ".*$\') )")
			...
			if ("addr_name" in args):
				whereClause.append("(`FQ`.`addr_name` REGEXP \'^.*" + args['addr_name'] + ".*$\')")

			#[begin] Build order by clause.
			ORDER_KEYS = {\
				"client_name": "`CL`.`name`", \
				"engineer_client_name": "`FQCL`.`name`", \
			...
			try:
				...
				result = Q_Model.convert("enum_purchases", dbcur)
				status['code'] = 0
	...
	def _fn_enum_purchases_pdfinfo(self, chain_env):
		time_bg = time.time()
		chain_env['logger']("webapi", ("BEGIN", chain_env['argument'].data))
		result = []
		status = {"code": None, "description": None}
		...
		if dbcur:
			...
			print whereValues
			try:
				dbcur.execute(Q_Model.sql("enum_purchases_pdf_info") % (("AND " + "\n AND ".join(whereClause)) if whereClause else "", ", ".join(orderClause)), param1)
				# dbcur.execute(Q_Model.sql("enum_estimates"))
			except:
				pprint.pprint(traceback.format_exc())
				chain_env['trace'].append(unicode(traceback.format_exc(), "utf8"))
