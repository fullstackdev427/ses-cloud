...
class Processor(ProcessorBase):
	...
	def _fn_html_top(self, chain_env):
		...
		#Fetch engineers.
		status_engineer, render_param['estimate.enumEstimates'] = self._fn_enum_estimates(chain_env)
	...
	def _fn_enum_estimates(self, chain_env):
		...
		if dbcur:
			...
			try:
				...
				result = Q_Model.convert("enum_estimates", dbcur)
				status['code'] = 0
	...
	def _fn_enum_estimates_pdfinfo(self, chain_env):
		time_bg = time.time()
		chain_env['logger']("webapi", ("BEGIN", chain_env['argument'].data))
		result = []
		status = {"code": None, "description": None}
		...
		if dbcur:
			...
			print whereValues
			try:
				dbcur.execute(Q_Model.sql("enum_estimates_pdf_info") % (("AND " + "\n AND ".join(whereClause)) if whereClause else "", ", ".join(orderClause)), param1)
				# dbcur.execute(Q_Model.sql("enum_estimates"))
			except:
				pprint.pprint(traceback.format_exc())
				chain_env['trace'].append(unicode(traceback.format_exc(), "utf8"))
