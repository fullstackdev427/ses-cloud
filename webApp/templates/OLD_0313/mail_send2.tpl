...
{% if "type_recipient" in query and (query.type_recipient == "forWorker") -%}
	{% set tpl_types = ("取引先担当者", "取引先担当者（既定）") -%}
{% elif "template_type" in query and query.template_type == "forEngineer" -%}
					...
					<div id="header" class="row">
						...
						<div class="col-md-12">
							...
							<ul class="list-inline">
								<li><p>提案区分によって色分けされます。</p></li>
								<li class='btn btn-job'>案件</li>
								<li class='btn btn-worker'>人材</li>
								<li class='btn btn-job_and_worker'>案件・人材</li>
                                {% if query.type_recipient == "forMatching" %}
                                <li class='btn btn-bp_user'>案件・人材公開企業</li>
                                {% endif %}
							</ul>
                            {% endif %}
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-footer">
				<a onclick="enumReserve();" style="cursor: pointer;">予約送信をしたメール数(<span id="reserve_count"></span>)</a>
				<button type="button" class="btn btn-default" data-dismiss="modal">閉じる</button>
				<button class="btn btn-success"
					data-toggle="popover"
					data-placement="top"
					data-content="送信日時を選択してから、送信を確定します。"
					onmouseover="$(this).popover('show');"
					onmouseout="$(this).popover('hide');"
					onclick="confirmReserve();">予約送信</button>
...
<!-- mail reserve dialog -->
<div id="set_reserve_modal" class="modal fade modal-sm" role="dialog" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<div class="modal-body">
					<div class="input-group">
						...
						<input type="datetime-local" id="reserve_date">
					</div>
					<div class="input-group">
						<span class="input-group-addon">件名</span>
						<input type="text" class="form-control" id="reserve_subject"/>
					</div>
					<div class="input-group">
						<span class="input-group-addon">本文</span>
						<span><textarea class="form-control" id="reserve_body" style="height: 30em;"></textarea></span>
					</div>
				</div>
				<div style="height: 2em;">
					<button type="button" class="btn btn-danger" style="float: left; margin-left: 20px;"
						onclick="if (!this.disabled) {goBack();}">戻る</button>
					<button type="button" class="btn btn-success" style="float: right; margin-right: 20px;"
						onclick="if (!this.disabled) {sendMailReserve(env.confirmedMailRequest);}">送信</button>
				</div>
			</div>
		</div>
	</div>
...
<!-- mail reserve dialog -->
<div id="enum_reserve_modal" class="modal fade modal-sm" role="dialog" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<div class="modal-body">
					<table class="view_table table-bordered table-hover">
						...
						<tbody id="tablebody_forReservedMails" style="height: 20em;"></tbody>
					</table>
				</div>
				<div style="height: 2em;">
					<button type="button" class="btn btn-success" style="float: right; margin-right: 20px;"
						onclick="if (!this.disabled) {closeEnumReserve();}">確認</button>
				</div>
			</div>
		</div>
	</div>
