...
<input type="hidden" id="company_id" value="{{ query.company_id}}">
<input type="hidden" id="engineer_id" value="{{ query.engineer_id}}">
<!-- メインコンテンツ -->
<div class="row">
    <div class="container" style="margin-bottom:100px;margin-top:30px;">
            ...
            <div class="container">
                <div class="row">
                    <div class="col-xs-2">
                        ...
                        <span id="quotation_header_label"></span>
                    </div>
                    <div class="col-xs-7">
        ...
        {% if data['operation.enumOperations'] %}
            {% if data["auth.userProfile"].company.id == data['operation.enumOperations'][0].engineer_company_id or (not data['operation.enumOperations'][0].engineer_company_id) %}
                <input type="hidden" id="company_user_ids" value=""/>
                <input type="hidden" id="client_worker_ids" value="{% if data['operation.enumOperations'] %}{{ data['operation.enumOperations'][0].engineer_worker_id_list|e }}{% endif %}"/>
            {% else %}
                <input type="hidden" id="company_user_ids" value="{% if data['operation.enumOperations'] %}{{ data['operation.enumOperations'][0].engineer_company_user_id_list|e }}{% endif %}"/>
                <input type="hidden" id="client_worker_ids" value=""/>
            {% endif %}
        {% else %}
            <input type="hidden" id="company_user_ids" value=""/>
            <input type="hidden" id="client_worker_ids" value=""/>
            ...
            <div class="col-xs-12" style="margin-left:40px;margin-right:30px;margin-top:30px;">
                <div class="row">
                    <div class="col-xs-12 text-left">
                        <input type="checkbox" id="is_view_window" onchange="viewAddrFormArea();"/>窓付き封筒用の宛先住所をつける
                        <div id="addr_form_area" style="display: none">
                            <table class="">
                                <tr>
                                    <td style=" width: 70pt; padding-left: 10pt; font-size: small"> 郵便番号</td>
                                    <td>
                                        <input type="text" class="" id="addr_vip" placeholder="nnn-nnnn" style="width: 8em;" maxlength="8"/>
                                        <span class="btn btn-sm btn-default" onclick="searchZip2Addr($('#addr_vip').val(), '#addr1', '#addr1_alert')"><span class="text-danger bold">〒</span>住所検索</span>
                                        <span class="text-danger" id="addr1_alert"></span>
                                    </td>
                                </tr>
                                <tr>
                                    <td style=" width: 70pt; padding-left: 10pt; font-size: small"> 住所</td>
                                    <td><input type="text" class="" id="addr1" style="width: 32em;" maxlength="64"/></td>
                                </tr>
                                <tr>
                                    <td style=" width: 70pt; padding-left: 10pt; font-size: small"> ビル名</td>
                                    <td><input type="text" class="" id="addr2" style="width: 32em;" maxlength="64"/></td>
                                </tr>
                            </table>
                        </div>
                    </div>
                ...
                <div class="row">
                    <div class="col-xs-12 text-center strong">
                        <br/><br/>
                        <h1><span id="quotation_title_label">注文書1</span></h1>
                    </div>
                </div>
                <div class="row">
                    <div class="col-xs-6">
                        <h3>
                          <!--20200723コメントアウト
                            <input type="text" class="" style="width:12em;" maxlength="64" id="addr_name" onchange="loadClientInfo(false);"/>
						    <input type="hidden" id="client_id"/>
                            <input type="hidden" class="" id="client_name"/>
                          -->
                          <!--20200723代わりに追加-->
                          <select type="" class="" style="width:12em;" maxlength="64" id="addr_name" onchange="loadClientInfo(false);"/>
                              <option>　</option>
                          ...
                          <input type="hidden" class="" id="client_name"/>
                          <!--end 20200723追加-->
                            <select id="honorific" value="" style="font-size: large;">
                    ...
                    <div class="col-xs-4">
                        ...
                        <p style="position: absolute; z-index: 2;">
                            ...
                            担当：{% if data['manage.readUserProfile'].company.purchase_charging_user_id %}
                                    {% for item in data['manage.enumAccounts'] %}
                                        {% if item.is_enabled == True %}
                                            {% if item.id == data['manage.readUserProfile'].company.purchase_charging_user_id %}{{ item.name|e }}{% endif %}
                                        {% endif %}
                                    {% endfor %}
                                {% endif %}
                        </p>
...
<!-- /メインコンテンツ -->
<!-- [begin] Modal. -->
<div id="search_operation_modal" class="modal fade" role="dialog" aria-hidden="true" data-keyboard="false" data-backdrop="static">
	<div class="modal-dialog" style="width:1000px;">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title"><span class="glyphicon glyphicon-search">&nbsp;</span>既存データから作成</h4>
			</div><!-- div.modal-header -->
			<div class="modal-body">
                <h4>検索条件</h4>
				<div id="modal_search_container_engineer" style="overflow: hidden;">
{#                <input type="hidden" id="query_is_active" value="1"/>#}
					<ul style="margin-top: 0.5em; padding: 0.3em 0.5em; background-color: #f1f1f1; list-style-type: none;/* font-size: 11px;*/ overflow: hidden;">
                        ...
                        <li style="margin: 1px 2em; float: left;">
                            <label for="query_project_title" style="color: #666666; width: 5em;">案件名</label>
                            <input type="text" id="query_project_title" value="{{ query.project_title|e }}"/>
                        </li>
                        <li style="margin: 1px 2em; float: left;">
                            <label for="query_charging_user_name" style="color: #666666; width: 5em;">営業担当</label>
...
{#	env.history = JSON.parse('{{ data['output_history']|tojson }}');#}
	...
	env.company_id = {% if data['company_id'] %}{{ data['company_id']}}{% else %}null{% endif %};
	if(env.company_id){
	    env.userCompany = {% if data['company_id'] %}JSON.parse('{{ data['operation.enumOperations']|tojson }}'){% else %}null{% endif %};
    }
