...
<!-- メインコンテンツ -->
	<div class="row">
		<div class="container" style="margin-bottom:100px;">
			...
			<div class="row">
				<div class="col-sm-2">
					<img alt="請求先注文書" width="22" height="20" src="/img/icon/group_case.png"> 請求先注文書 一覧
				</div>
			</div>
			<div class="row" style="/*background-color: #f1f1f1;*/">
				<form onsubmit="c4s.hdlClickSearchBtn(); return false;">
					...
					<ul style="margin-top: 0.5em; padding: 0.3em 0.5em; background-color: #f1f1f1; list-style-type: none;/* font-size: 11px;*/ overflow: hidden;">
                        ...
                        <li style="margin: 0 2em; float: left;">
							<label for="query_quotation_month" style="color: #666666;">請求月</label>
			...
			<div class="row" style="margin-top:20px;margin-bottom:20px;">
				<div class="col-lg-7">
                    <span class="btn" onclick="triggerCreateQuotationOrder()">新規作成&nbsp;<span class="glyphicon glyphicon-plus-sign"></span></span>
				</div>
				<!-- 件数 -->
				{{ buttons.paging(query, env, data['order.enumOrders']) }}
				<!-- /件数 -->
			</div>
			<!-- /検索結果ヘッダー -->
			<!-- 検索結果 -->
...
{#							<th style="width: 35px;">選択<br/><input type="checkbox" id="iter_estimate_selected_cb_0" onclick="c4s.toggleSelectAll('iter_operation_selected_cb_', this);"/></th>#}
                            ...
                            <th>{{ buttons.th(query, '顧客名（注文元企業）', 'client_name') }}</th>
{#                            <th>{{ buttons.th(query, '注文案件', 'project_title') }}</th>#}
					...
					<tbody>
                    {% if data['order.enumOrders'] %}
						{% set row_min = env.limit.ROW_LENGTH * ((query.pageNumber or 1) - 1) %}
						{% set items = data['order.enumOrders'][row_min:row_min + env.limit.ROW_LENGTH] %}
						{% for item in items %}
                        <tr id="iter_{{ item.id }}">
...
{#                            </td>#}
                            <td style="text-align: center">
                                <span class="pseudo-link-cursor glyphicon glyphicon-file text-primary" alt="　請求先注文書/編集" title="請求先注文書/編集"
