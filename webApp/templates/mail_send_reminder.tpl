...
{% if "type_recipient" in query and query.type_recipient == "forMember" -%}
	{% set tpl_types = ("リマインダー") -%}
{% endif -%}
			...
			<div class="row">
				<div {% if "iPhone" in env.UA or "Android" in env.UA -%}class="sp_content" style="margin-top: 1em;" {% else -%}class="container" style="margin-bottom:100px;"{% endif -%}>
					...
					<div id="header" class="row">
						...
						<div class="col-md-12">
							...
							<div class="input-group">
								...
								{% if query.type_recipient == "forMember" %}
									<li class="btn btn-sm btn-primary pull-right bold" style="text-align: right;"
										onclick="openRecipientsModalOnReminder('member');">追加
									</li>
								{% endif %}
								</ul>
							</div>
					...
					<div class="">
						<div class="col-md-7">
							<h4>メール内容情報&nbsp;
							{% if query.type_recipient == "forWorker" %}
								（取引先担当者向けテンプレート）
							{% elif query.type_recipient == "forEngineer" %}
								（技術者向けテンプレート）
							{% elif query.type_recipient == "forMember" %}
								（リマインダー）
							{% endif %}
						...
						{% if data['mail.enumTemplates']|length == 1 %}
							{% set item = data['mail.enumTemplates'][0] %}
							{% set tpl_id = item.id %}
							{% set attachments = item.attachments %}
						{% endif %}

							<input type="hidden" id="input_0_tpl_id_1" value="{{ tpl_id }}"/>
							<div class="input-group" style="display: none;">
								<span class="input-group-addon">敬称</span>
								<ul class="form-control" style="list-style-type: none;">
							{% for title in title_types %}
									<li>
										<input type="radio" name="input_0_type_title_{{ tpl_id }}" id="input_0_type_title_{{ tpl_id }}_1" value="{{ title|e }}"{% if loop.index0 == 0 %} checked="checked"{% endif %}/><label for="input_0_type_title_{{ tpl_id }}_1">{{ title|e }}</label>
									</li>
							{% endfor %}
								</ul>
							</div>
							<div class="input-group">
								...
								<input type="text" class="form-control" id="input_0_subject_1" value="{{ item.subject|e }}"/>
							</div>
							<div class="input-group">
								<span class="input-group-addon">本文</span>
								<div class="form-control">
									<span><textarea class="form-control" id="input_0_body_1" style="height: 30em;">{{ item.body|e }}</textarea></span>
									{#{% if item.type_iterator|length > 0 %}#}
									<button class="popover-dismiss btn btn-default bold"
										id="input_0_refresh_btn_1"
										data-toggle="popover"
										data-placement="top"
										data-content="データをテンプレートに再反映させ、編集内容をクリアします。"
										onmouseover="$(this).popover('show');"
										onmouseout="$(this).popover('hide');"
										onclick="refreshMailBody({ template_id:{{ tpl_id }}, type_title: ($('#input_0_type_title_{{ tpl_id }}_1')[0].checked ? '様' : 'さん'), body_id: 'input_0_body_1'});">
										<span class="glyphicon glyphicon-refresh text-success"></span>&nbsp;
										差し込みデータを本文に反映
									</button>{% if env.limit.SHOW_HELP %}<span class="text-warning">右表で選択し、左のボタンで本文に反映させます。</span>{% endif %}
									{#{% endif %}#}
								</div>
							</div>
							<div class="input-group">
								...
								{% for idx in range(0, env.limit.LMT_LEN_MAIL_ATTACHMENT, 1) %}
									...
									<li class="list-group-item" id="attachment_container_{{ tpl_id }}_{{ idx }}" style="overflow: hidden;">
										<input type="hidden" id="attachment_id_{{ tpl_id }}_{{ idx }}" value="{% if attachment|e %}{{ attachment.id }}{% endif %}"/>
										<label id="attachment_label_{{ tpl_id }}_{{ idx }}"
											class="bold mono pseudo-link"
											style="display: none;"
											onclick="c4s.download($('#attachment_id_{{ tpl_id }}_{{ idx }}').val());"></label>
							...
							<div style="height: 2em;">
								<button class="pull-right popover-dismiss btn btn-primary bold"
									...
									onmouseover="$(this).popover('show');"
									onmouseout="$(this).popover('hide');"
									onclick="confirmBody({template_id: {{ tpl_id }}, template_type: '{{ item.type_recipient|e }}', type_title: ($('input[id^=input_0_type_title_{{ tpl_id }}_]')[0].checked ? '様' : 'さん'), subject: $('#input_0_subject_1').val(), body_id: '#input_0_body_1', attachment_id: [{% for idx in range(0, env.limit.LMT_LEN_MAIL_ATTACHMENT, 1) %}'#attachment_id_{{ tpl_id }}_{{ idx }}'{% if loop.index < env.limit.LMT_LEN_MAIL_ATTACHMENT %}, {% endif %}{% endfor %}]});"><span class="glyphicon glyphicon-ok"></span>&nbsp;確認</button>
							</div>
						...
						<div class="col-md-5">
							<ul class="nav nav-tabs" role="tablist">
								<li>
									<a href="#dataview_engineers" role="tab" data-toggle="tab">要員情報</a>
								</li>
								<li>
									...
									<table class="view_table table-bordered table-hover">
										<thead>
											<tr>
												<th style="width: 25px;">
													<input type="checkbox"
														onclick="c4s.toggleSelectAll('iter_engineer_', this);"/>
												</th>
												<th>要員名</th>
												<th>単価</th>
												<th>稼働</th>
									...
									<table class="view_table table-bordered table-hover">
										<thead>
											<tr>
												<th style="width: 25px;">
													<input type="checkbox"
														onclick="c4s.toggleSelectAll('iter_project_', this);"/>
												</th>
												<th>案件名</th>
												<th>請求</th>
												<th>期間</th>
	...
	<div class="modal-dialog">
		<div class="modal-content">
					...
					<ul style="padding: 0; background-color: #ebebeb; list-style-type: none; overflow: hidden;">
						...
						<li style="margin: 3px 5px; float: left;">
							...
							<select id="modal_search_charging_worker">
								<option value=""></option>
							{% for user in data['manage.enumAccounts'] %}
			...
			<div class="modal-footer">
				<button type="button" class="btn btn-default" data-dismiss="modal">閉じる</button>
				<button type="button" class="btn btn-primary" id="input_account_btn"
					onclick="hdlClickAddRecipientBtnOnEdit('member');">まとめて追加</button>
			</div><!-- div.modal-footer -->
		</div><!-- div.modal-content -->
	</div><!-- div.modal-dialog -->
</div><!-- div.modal -->
