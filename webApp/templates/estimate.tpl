...
<!-- メインコンテンツ -->
	<div class="row">
		<div class="container" style="margin-bottom:100px;">
			...
			<div class="row">
				<div class="col-sm-2">
					<img alt="見積書" width="22" height="20" src="/img/icon/group_case.png"> 見積書 一覧
				</div>
			</div>
			<div class="row" style="/*background-color: #f1f1f1;*/">
				<form onsubmit="c4s.hdlClickSearchBtn(); return false;">
					...
					<ul style="margin-top: 0.5em; padding: 0.3em 0.5em; background-color: #f1f1f1; list-style-type: none;/* font-size: 11px;*/ overflow: hidden;">
                        ...
                        <li style="margin: 0 2em; float: left;">
							<label for="query_quotation_month" style="color: #666666;">見積月</label>
			...
			<div class="row" style="margin-top:20px;margin-bottom:20px;">
				<div class="col-lg-7">
                    <span class="btn" onclick="triggerCreateQuotationEstimate()">新規作成&nbsp;<span class="glyphicon glyphicon-plus-sign"></span></span>
				</div>
				<!-- 件数 -->
				{{ buttons.paging(query, env, data['estimate.enumEstimates']) }}
				<!-- /件数 -->
			</div>
			<!-- /検索結果ヘッダー -->
			<!-- 検索結果 -->
...
{#                            <th>{{ buttons.th(query, '見積案件', 'project_title') }}</th>#}
					...
					<tbody>
                    {% if data['estimate.enumEstimates'] %}
						{% set row_min = env.limit.ROW_LENGTH * ((query.pageNumber or 1) - 1) %}
						{% set items = data['estimate.enumEstimates'][row_min:row_min + env.limit.ROW_LENGTH] %}
						{% for item in items %}
                        <tr id="iter_{{ item.id }}">
{#                            <td class="text-center">#}
{#                                <input type="checkbox" id="iter_estimate_selected_cb_v" value=""/>#}
{#                            </td>#}
                            <td style="text-align: center">
                                <span class="pseudo-link-cursor glyphicon glyphicon-file text-primary" alt="　見積書出力/編集" title="見積書出力/編集"
