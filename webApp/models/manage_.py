...
"enum_users": """SELECT
  ...
  INNER JOIN `mt_user_groups` AS `G`
    ON `G`.`id` = `P`.`group_id`
  WHERE `P`.`id` IN (%s);""",\
"enum_prefs": """SELECT `owner_id`, `key`, `value`, `marshal` FROM `ft_prefs` WHERE `owner_id` IN (0, valid_user_company(%s, %s, %s));""", \
...
"insert_mt_quotation_config": """
    ...
    company_id, company_seal, bank_account1, bank_account2,\
     estimate_charging_user_id, order_charging_user_id, purchase_charging_user_id, invoice_charging_user_id) VALUES 
	((SELECT id FROM mt_user_companies WHERE `prefix` = %s) ,%s,%s,%s,%s,%s,%s,%s);""", \
 ...
 \
	...
	def __cvt_read_user_profile(cls, cur):
		for res in cur:
			...
			tmp['company']['bank_account1'] = res[32]
			tmp['company']['bank_account2'] = res[33]
			tmp['company']['estimate_charging_user_id'] = res[34]
			tmp['company']['order_charging_user_id'] = res[35]
			tmp['company']['purchase_charging_user_id'] = res[36]
			tmp['company']['invoice_charging_user_id'] = res[37]
			return tmp
