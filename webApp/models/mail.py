...
INSERT INTO `ft_mails` (
  ...
  %s,
  valid_user_id_full(%s, %s, %s));""",\
"save_mail_reserve_time": u"""\
INSERT INTO `ft_mails_time`
(`mail_req_id`, `send_time`, `user_profile`, `reply_to`, `x_gwcustom_reply`) VALUES (
%s,
%s,
%s,
%s,
%s);""",\
"get_current_reserve": """\
  SELECT `t`.`id`, `t`.`mail_req_id`, `t`.`send_time`, `t`.`user_profile`, `t`.`reply_to`, `t`.`x_gwcustom_reply`,
  `m`.`addr_to`, `m`.`addr_cc`, `m`.`addr_bcc`, `m`.`id_replyto`, `m`.`type_title`, `m`.`tpl_id`, `m`.`subject`, `m`.`body`, `m`.`creator_id`
  from `ft_mails_time` `t` LEFT JOIN `ft_mails` `m` ON `t`.`mail_req_id` = `m`.`id` WHERE `t`.mail_sent = 0 AND `t`.send_time <= %s;""",\
"get_reserved_mails_count": """\
  SELECT COUNT(*) from `ft_mails_time` `t` 
  LEFT JOIN `ft_mails` `m` ON `t`.`mail_req_id` = `m`.`id` WHERE `t`.mail_sent = 0 AND `m`.`creator_id` = %s;""",\
"enum_reserve": """\
  SELECT `t`.`id`, `t`.`mail_req_id`, DATE_FORMAT(`t`.`send_time`, '%%m/%%d/%%Y %%H:%%i') AS send_time_str, `t`.`user_profile`,
  `m`.`subject`, `m`.`creator_id`
  from `ft_mails_time` `t` LEFT JOIN `ft_mails` `m` ON `t`.`mail_req_id` = `m`.`id` WHERE `t`.mail_sent = 0 AND `m`.`creator_id` = %s;""",\
"get_reserve_info": """\
  SELECT `t`.`id`, `t`.`mail_req_id`, DATE_FORMAT(`t`.`send_time`, '%%Y-%%m-%%dT%%H:%%i') AS send_time_str, `t`.`user_profile`,
  `m`.`subject`, `m`.`body`, `m`.`creator_id`
  from `ft_mails_time` `t` LEFT JOIN `ft_mails` `m` ON `t`.`mail_req_id` = `m`.`id` WHERE `t`.`id` = %s;""",\
"delete_reserve": """\
  DELETE FROM `ft_mails_time` WHERE `id` = %s;""",\
"update_reserve_sent": """\
  UPDATE `ft_mails_time` SET `mail_sent` = 1 WHERE `id` = %s;""",\
...
"last_insert_id": """SELECT LAST_INSERT_ID();"""\
	...
	@classmethod
	def __cvt_get_reserved_mails_count(cls, cur):
		result = 0
		for res in cur:
			result = res[0]
		return result
	...
	@classmethod
	def __cvt_get_current_reserve(cls, cur):
		result = []
		for res in cur:
			tmp = {}
			tmp['id'] = res[0]
			tmp['mail_req_id'] = res[1]
			tmp['send_time'] = res[2]
			tmp['user_profile'] = JSON.loads(res[3])
			tmp['reply_to'] = res[4]
			tmp['x_gwcustom_reply'] = res[5]
			tmp['addr_to'] = res[6]
			tmp['addr_to_list'] = JSON.loads(res[6])
			tmp['addr_cc'] = res[7]
			tmp['addr_bcc'] = res[8]
			tmp['id_replyto'] = res[9]
			tmp['type_title'] = res[10]
			tmp['tpl_id'] = res[11]
			tmp['subject'] = res[12]
			tmp['body'] = res[13]
			tmp['creator_id'] = res[14]
			result.append(tmp)

		return result

	@classmethod
	def __cvt_enum_reserve(cls, cur):
		result = []
		for res in cur:
			tmp = {}
			tmp['id'] = res[0]
			...
			tmp['user_profile'] = JSON.loads(res[3])
			tmp['subject'] = res[4]
			tmp['creator_id'] = res[5]
			result.append(tmp)

		return result

	@classmethod
	def __cvt_get_reserve_info(cls, cur):
		result = []
		for res in cur:
			tmp = {}
			tmp['id'] = res[0]
